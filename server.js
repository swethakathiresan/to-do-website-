...
function formatDate(date) {
    if (!date || isNaN(Date.parse(date))) {
        console.error("Invalid date value passed to formatDate:", date); 
        return "N/A";
    }
...
app.post('/tasks', (req, res) => {
    ...
    db.query(query, [title, description, due_date, status], (err, result) => {
        if (err) {
...
app.put('/tasks/:id', (req, res) => {
    const { id } = req.params;
    const { title, description, due_date, status, createdAt, deleted } = req.body;
